...
* Move Paparazzi configuration modifications out of snapshot/gif into unsafeUpdateConfig method
* Surface internally thrown exceptions from LayoutLib
* Throw a more helpful exception if Android platform is missing
* Cache initial Renderer and method interception bootstrap logic for better performance
...
* Exclude androidTest sourceSets during paparazzi runs
* Update default SDK path on Linux
* Fix accessibility test logic to avoid unnecessary coloring changes on layout changes
* Publish plugin marker on snapshot builds
